...
#         )
#         self.dd = nn.Dropout()
#         self.cc = nn.Linear(4096, num_classes)
#         self._initialize_weights()
...
#         self.linear00 = nn.Linear(512, 1024)
#         self.linear = nn.Linear(512*7*7, 4096)
#         self.linear1 = nn.Linear(4096, 4096)
#         self.classifier00 = nn.Linear(1024, num_classes)
#         self.classifier000 = nn.Linear(512, num_classes)
...
def vgg16_dcf(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(make_layers_dcf(cfg['D']), **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['vgg11']))
    return model
